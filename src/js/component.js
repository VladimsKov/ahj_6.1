...
export default class CardSys {
  constructor() {
    ...
    for (const elem of this.addBtns) {
  ...
  static freeInputFieldSet(container, evt, ghostEl) {
    ...
    if (closestEl.classList.contains('card')) {
      ...
      if (evt.pageY > window.scrollY + top + closestEl.offsetHeight / 2) {
        ...
        // запоминаем пустое поле вставки, которое ниже курсора относительно closestEl:
        inputFieldCoords.top = closestEl.getBoundingClientRect().bottom;
        inputFieldCoords.bottom = closestEl.getBoundingClientRect()
          .bottom + ghostEl.offsetHeight + 10;
        inputFieldCoords.pos = 'bottom';
      } else {
        // добавить отступы для поля вставки выше курсора относительно closestEl:
